...
export const createNotificationRouter = (scheduler: NotificationScheduler, logger: Logger) => {
  const router = Router();

  router.post("/api/notifications", async (req: Request, res: Response) => {
    logger.log(`Received a POST. \n\tEndpoint: ${req.url}, \n\tBody: ${JSON.stringify(req.body)}`);
  ...
  router.get("/api/notifications/:notificationId", async (req: Request, res: Response) => {
    logger.log(
  ...
  router.patch("/api/notifications/:notificationId", async (req: Request, res: Response) => {
    logger.log(
      `Received a PATCH. \n\tEndpoint: ${req.url}, \n\tParams: ${JSON.stringify(req.params)}`,
    );
